...
def streaming_sparse_attn_varlen_func_kernel(num_heads, dim_qk, dim_vo, softmax_scale, block_M=64, block_N=128, num_stages=1, threads=128):
...
def streaming_sparse_attn_varlen_func(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    kernel = streaming_sparse_attn_varlen_func_kernel(
...
def streaming_sparse_attn_with_kvcache_kernel(num_seqs, seqlen_q, num_heads, num_kv_heads, dim_nope, dim_rope, softmax_scale, num_pages, page_size, block_H=64, block_N=128, num_splits=1, num_stages=1, threads=128):
    # num_seqs = T.dynamic("num_seqs")
...
def streaming_sparse_attn_with_kvcache(
    q: torch.Tensor,
    kv: torch.Tensor,
    ...
    block_H = 64
    block_N = 128
    num_splits = get_splits(batch_size, num_heads_q, seqlen_q, torch.mean(cache_seqlens.float()).int().item(), block_H, block_N, [1, 7])
    num_stages = 1
    threads = 256
    ...
    kernel = streaming_sparse_attn_with_kvcache_kernel(
